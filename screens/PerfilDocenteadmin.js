...
export default class PerfilDocenteadmin extends Component {
  constructor(props) {
    super(props);
    this.state = {
      item: [],
  ...
  async update(item) {
    ...
    try {
      ...
      if (response.status >= 200 && response.status < 300) {
        Alert.alert("Docente alterado com sucesso!!!");
      }
    } catch (errors) {
      Alert.alert(errors);
    }
      ...
      <View style={styles.container}>
        <ImageBackground source={imagem} style={styles.imageFundo}>
          ...
          >
            ...
            <View style={{ flexDirection: "row" }}>
              <Text style={styles.informacoes}>Função:</Text>
              <TextInput
                style={styles.input}
                autoCorrect={false}
                onChangeText={() => {}}
          ...
          <View style={styles.viewbutton}>
            <Button
              title="Salvar"
              color="#870B5A"
              onPress={() => this.update(this.state.item)}
            ></Button>
          </View>
        </ImageBackground>
      </View>
