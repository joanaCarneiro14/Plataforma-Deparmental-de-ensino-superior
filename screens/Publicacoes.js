...
export default class Publicacoes extends Component {
  ...
  async deletepublicacao(section) {
    const url = "http://194.210.110.169/projeto/web/rest/publicacao/delete?id=";
  ...
  async componentDidMount() {
    ...
    const autoria = await fetch(
      "http://194.210.110.169/projeto/web/rest/autoria-publicacao"
    ).then((response) => response.json());
    console.log("autoria", autoria);
    const publicacaosautoria = autoria
      .filter((ap) => {
        if (ap.id_autor == docente.id) {
          return ap;
        }
      })
      .map((item) => item.id_publicacao);
    console.log(publicacaosautoria);

    const publicacoes = await fetch(
      "http://194.210.110.169/projeto/web/rest/publicacao"
    ).then((response) => response.json());

    const publicacoesfiltrados = publicacoes.filter((publicacao) => {
      if (publicacaosautoria.includes(publicacao.id)) {
        return publicacao;
      }
    });

    this.setState({
      publicacoes: publicacoesfiltrados,
    });
    console.log("result", publicacoesfiltrados);
  }
      ...
      >
        <View style={{ flexDirection: "row" }}>
          ...
          >
            ...
            <Text>
              Data de finalização: {section.data_finalizacao} {"\n"}
            </Text>
            <Text>
            ...
            >
              <Button
                ...
                color="#870B5A"
                onPress={() =>
                  this.props.navigation.navigate("EditarPublicacao", {
                    item: section,
                  })
                }
              ></Button>
            ...
            >
              <Button
                title="Excluir"
                color="#870B5A"
                onPress={() => this.deletepublicacao(section)}
              ></Button>
            </View>
          </Animatable.Text>
        </View>
        ...
        <ImageBackground source={imagem} style={styles.imageFundo}>
          <View style={{ flexDirection: "row" }}>
            ...
            >
              <Button
                ...
                color="#870B5A"
                onPress={() =>
                  this.props.navigation.navigate("AdicionarPublicacao")
                }
              ></Button>
            </View>
          </View>
          <ScrollView contentContainerStyle={{ paddingTop: 30 }}>
            <Accordion
              ...
              sections={this.state.publicacoes}
