...
export default class EscolaESA extends Component {
  ...
  async componentDidMount() {
    const docente = await fetch(
      "http://194.210.110.179/projeto/web/rest/docente/view?id=46"
    ).then((response) => response.json());
    this.setState({
      diretornome: docente,
    });
    const utilizador = await fetch(
      "http://194.210.110.179/projeto/web/rest/utilizador/view?id=28"
    ).then((response) => response.json());
    this.setState({
      diretoremail: utilizador,
    });
    const escola = await fetch(
      "http://194.210.110.179/projeto/web/rest/escola/view?id=27"
    ).then((response) => response.json());
    this.setState({
      escola: escola,
    })
        ...
        <ImageBackground source={imagem} style={styles.imageFundo}>
          ...
          <View style={styles.viewbutton}>
            <Button
              title="Editar"
              color="#870B5A"
              onPress={() => this.props.navigation.navigate("EditarESA")}
            ></Button>
          </View>
        </ImageBackground>
      </View>
